# RegisterAuthAddressDeveloperManagedSignedKeyReqBody


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**address** | **string** | Ethereum address | [default to undefined]
**app_fid** | **number** | The unique identifier of a farcaster user or app (unsigned integer) | [default to undefined]
**deadline** | **number** | unix timestamp in seconds that controls how long the signed key request is valid for. (24 hours from now is recommended) | [default to undefined]
**signature** | **string** | Signature generated by the custody address of the app. Signed data includes app_fid, deadline, 32 bytes padded auth address. [Refer guide for more details.](https://docs.neynar.com/docs/auth-address-signature-generation) | [default to undefined]
**redirect_url** | **string** | Url to redirect to after the signer is approved. **Note** : This should only be used when requesting a signer from a native mobile application. | [optional] [default to undefined]
**sponsor** | [**SignedKeyRequestSponsor**](SignedKeyRequestSponsor.md) |  | [optional] [default to undefined]

## Example

```typescript
import { RegisterAuthAddressDeveloperManagedSignedKeyReqBody } from './api';

const instance: RegisterAuthAddressDeveloperManagedSignedKeyReqBody = {
    address,
    app_fid,
    deadline,
    signature,
    redirect_url,
    sponsor,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
